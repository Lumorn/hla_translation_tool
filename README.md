...
* **Schneller Zugriff:** Die Schnellzugriffsleiste bleibt eine kompakte Toolbar mit kurzen Labels direkt neben den Icons. Trim ✂️, Auto ⚡, Tempo ⏱️, Pegel 🔊 und Funk 📻 behalten dabei wieder ihre ursprüngliche Höhe, weil alle Buttons einzeln ausgerichtet werden und nicht mehr vom Tempo-Block gestreckt werden. Der Tempo-Knopf ist weiterhin genauso hoch wie die übrigen Buttons, jetzt jedoch mit einer einheitlichen 160‑px-Flexbasis, die für alle sechs Hauptbuttons gilt. Statt unter 1000 px umzubrechen, bleiben die Buttons nun stets in einer Reihe, lassen sich horizontal scrollen und halten durchgehend den Abstand aus dem `gap`. Auf sehr schmalen Displays greifen Scroll-Snap-Punkte, damit jeder Button sauber im Fokus einrastet.
...
* **Scroll-Toolbar-Screenshot:** `docs/screenshots/de-audio-quick-toolbar-scroll.txt` beschreibt das Artefakt `de-audio-quick-toolbar-scroll.png` mit einer einreihigen Schnellzugriffsleiste, sichtbarem Scrollbalken und eingerasteten Buttons über Scroll-Snap.
