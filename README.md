...
4. Alternativ kann `start_tool.bat` unter Windows oder `start_tool.js` (plattformunabhängig) genutzt werden. Ist Node installiert, lässt sich `start_tool.js` auch per Doppelklick starten. Fehlt das Repository, wird es automatisch geklont; andernfalls werden die neuesten Änderungen geladen und die Desktop-App gestartet
