...
* V3 ergänzt eine Explorer-Split-View mit Ordnerbaum für `source_audio/`, Pfadfilter, Zeilenzähler und Fertig-Häkchen für vollständige Ordner.
...
* Reset-Dialoge, Effektlisten, Statusmeldungen und Fehlertoasts im DE-Audio-Editor sind vollständig lokalisiert und verwenden dieselben `deAudio.*`-Schlüssel wie die HTML-Beschriftungen.
...
* GPT-Zielsprachen koppeln sich an die UI-Sprache: `language.defaultTarget` liefert „Deutsch“ oder „German“, Emo-Buttons, Dropdowns und GPT-Prompts nutzen diese Übersetzung und fallen andernfalls auf den gespeicherten Sprachcode zurück.
...
* Der DE-Audio-Editor öffnet auch ohne vorhandenen Cache-Eintrag, weil fehlende Pfade automatisch mit dem Standardordner ergänzt werden.
...
* **Einklappbare Audio-Zusammenfassung:** Der Info-Bereich startet eingeklappt, lässt sich per Klick aufklappen und schafft damit mehr Platz für die Wellenformen, wenn die technischen Details gerade nicht benötigt werden.
...
* **Tempo vor Effekten im DE-Editor:** `recomputeEditBuffer()` und `applyDeEdit()` schneiden und stretchen das Signal jetzt vor allen Funk-, Hall- und Störgeräuschfiltern, sodass Tempoänderungen auf dem hochauflösenden Material laufen und Wellenformlänge wie Wiedergabe stabil bleiben.
...
* **Extra-kompakte Tempo-Sonderleiste:** Minus-, Plus- und Preset-Knöpfe des Turbo-Blocks schrumpfen auf Miniformat, sodass der gesamte Zusatzblock nicht höher als ein einzelner Schnellzugriff-Button wird.
* **Segment-Vorschläge im DE-Editor:** Der neue Schnellzugriff „Segmente“ startet eine automatische Analyse, schreibt erkannte Stille als Vorschläge in eine eigene Karte, färbt sie direkt in Waveform und Timeline ein und bietet pro Segment Behalten-/Löschen-Toggles. Ein konfigurierbarer Sicherheits-Puffer, Sammelknöpfe „Alle behalten/löschen“ sowie das Live-Dragging der Segmentgrenzen übertragen Entscheidungen sofort in die Ignorierbereiche.
* **Zurückgesetzte Segment-Vorschläge nach dem Speichern:** Sobald eine Bearbeitung gesichert wird, leert der Editor automatisch alle Segment-Vorschläge inklusive Entscheidungen, aktualisiert die Vorschlagsliste und spiegelt die Änderungen umgehend in der Timeline wider.
...
* **Fallbacks für Reset- und Ladehelfer:** `selectProject`, `resetGlobalState` und `loadProjects` prüfen optionale Funktionen wie `stopEnglishReview`, `cancelTranslationQueue` und `applyDelayedTranslations`, wodurch Tests mit Teilimports nicht mehr an fehlenden Globals scheitern.
...
* **Tests für Segment-Vorschläge:** Validieren Padding, Sortierung und die Übernahme automatischer Ignorierbereiche der neuen Vorschlagslogik im DE-Editor.
...
* **Gezielter Zoo-Lautsprecher-Ton:** Der WaveShaper sitzt nun unmittelbar hinter der Monoquelle (inklusive sanfter Vorbetonung), anschließend klemmen zwei Hochpässe, zwei Tiefpässe und der Mitten-Boost vor einem strafferen Kompressor. Die neue Reihenfolge jagt Pegelspitzen zuerst durch die Vor-Sättigung und hält den Kompressor frei von hartem Clipping bei unveränderten Filterschwerpunkten.
...
* **Hiss & Brumm steuerbar:** Der Zoo-Lautsprecher mischt ein bandbegrenztes Rauschbett plus optionale 50/60 Hz-Sinusanteile per zusätzlichem OfflineAudioContext hinzu; Low-/Highcut und Pegel (z. B. −40 dB) lassen sich über das Parameterobjekt `zooSpeakerNoiseLevel` für künftige UI-Regler anpassen und definieren einen reproduzierbaren Noise-Floor von rund −74 dBFS in Signalpausen.
...
* **Schneller Zugriff:** Die Schnellzugriffsleiste verteilt jetzt alle sechs Hauptbuttons dynamisch über `flex: 1 1 calc((100% - GAP) / 6)` und hält sie in jeder Auflösung in einer Reihe. Gestaffelte `min-width`- und `max-width`-Grenzen für 1200 px, 1000 px und 600 px sorgen dafür, dass die Buttons gleichmäßig schrumpfen, in der Höhe unverändert bleiben und keinen Scrollbalken mehr benötigen. Auf sehr schmalen Displays blendet die Leiste weiterhin nur die Textlabels aus; die Icons rücken enger zusammen und behalten durch das einheitliche Gap ihre Ausrichtung.
* **Turbo-Feinsteuerung:** Direkt unter dem Tempo-Button liegt weiterhin die schmale Icon-Leiste. Minus, Anzeige und Plus teilen sich nun eine eigene Reihe in voller Tempo-Breite, während die Presets 1,20×/1,30× darunter einzeln stehen. So bleibt der Zusatzblock exakt so breit wie der Tempo-Knopf, ohne die orangenen Schnellzugriffsknöpfe aufzuziehen, und übernimmt weiterhin dessen Farbverlauf.
* **Aktueller Tempo-Screenshot:** `docs/screenshots/de-audio-turbo.txt` verlinkt das Artefakt `de-audio-quick-actions-focus.png`, das die kompakte Schnellzugriff-Leiste mit dem flachen Tempo-Block zeigt.
* **Responsive Toolbar-Screenshot:** `docs/screenshots/de-audio-quick-toolbar-responsive.txt` beschreibt das Artefakt `de-audio-quick-toolbar-responsive.png` mit einer scrollbarfreien Schnellzugriffsleiste, identischen Buttonbreiten und einer bündigen Tempo-Sondersteuerung.
* **Segment-Vorschlags-Screenshot:** `docs/screenshots/de-audio-segment-vorschlaege.txt` beschreibt das Artefakt `de-audio-segment-vorschlaege.png` mit aktivierter Vorschlagskarte, farbigen Overlays und Timeline-Balken.
...
* **Stille-Bereiche bleiben konsistent:** Eingefügte Stille wird nach einem erfolgreichen Speichern komplett zurückgesetzt, UI-Listen und Wellenform zeigen sofort den bereinigten Zustand. Fehlschläge oder Abbrüche stellen die vorherigen Stille-Markierungen automatisch wieder her.
...
23. In `requirements.txt` gekennzeichnete Zeilen mit `# optional` werden bei `verify_environment.py` nur informativ geprüft und lassen den Test bestehen; selbst im Reparaturmodus installiert das Skript ausschließlich Pflichtpakete und listet optionale Abhängigkeiten nur im Bericht.
...
Die Übersetzung entfernt automatisch führende Sprecherpräfixe (z. B. `NAME:` oder Anführungszeichen), sofern der Eingabetext kein entsprechendes Präfix hatte. Der Worker protokolliert entfernte Präfixe auf `stderr`, damit die Bereinigung nachvollziehbar bleibt.
...
* **📥 Daten importieren**
  ...
  * **Vollständig lokalisierter Import-Dialog:** Soundevents-Tabelle, Buttons, Statusmeldungen, Hinweise sowie Spinner-Text greifen komplett auf i18n-Schlüssel zurück und wechseln sofort zwischen Deutsch und Englisch.
  ...
  * **Mehrsprachiger Spaltendialog:** `data-i18n`-Attribute für Buttons, Labels, Hinweise und Auswahllisten sorgen dafür, dass CSV/Excel-Zuordnungen komplett in Deutsch oder Englisch angezeigt werden.
...
Über den Backup-Dialog kannst du jetzt einen **Struktur-Blueprint** exportieren: Die Funktion sammelt nur Kapitel, Level, Dateinamen und EN-Texte, entfernt sämtliche DE-Einträge sowie Audiopfad-Informationen und speichert das Ergebnis als schlanke JSON-Datei. Dabei wird auch die Level-Reihenfolge (`levelOrders`) festgehalten. Die Import-Schaltfläche lädt diesen Blueprint wieder ein, überschreibt die aktuellen Projekte und legt eine leere, DE-freie Arbeitsgrundlage an. Beim Einlesen werden sowohl die bekannten Kapitel-Reihenfolgen beibehalten als auch neue Level mit ihrer importierten Reihenfolge (oder ersatzweise dem Import-Index) im Tool gespeichert.
