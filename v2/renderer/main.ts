...
const dashboardSummaryElement = document.getElementById('project-dashboard-summary');
const dashboardListElement = document.getElementById('project-dashboard-list');
const dashboardEmptyElement = document.getElementById('project-dashboard-empty');
const dashboardFiltersElement = document.getElementById('project-dashboard-filters');
const dashboardRandomButton = document.getElementById('project-dashboard-random') as HTMLButtonElement | null;
const dashboardRandomReviewButton = document.getElementById('project-dashboard-random-review') as HTMLButtonElement | null;
...
const DEFAULT_CHAPTER_LABEL = 'Unsortiert';
const DEFAULT_LEVEL_LABEL = 'Allgemein';

type DashboardFilter = 'all' | 'open' | 'review' | 'done';
...
let dashboardContextMenu: HTMLDivElement | null = null;
...
function sanitizeChapterLabel(value: string | undefined): string {
...
function sanitizeLevelLabel(value: string | undefined): string {
  if (typeof value === 'string') {
    const trimmed = value.trim();
    if (trimmed.length > 0) {
      return trimmed;
  ...
  return DEFAULT_LEVEL_LABEL;
}

function normalizeProgressSnapshot(progress?: ProjectProgressSnapshot): ProjectProgressSnapshot {
  const safeNumber = (value: unknown): number => (typeof value === 'number' && Number.isFinite(value) ? value : 0);

  const statusCounts: Record<string, number> = {};
  if (progress?.statusCounts && typeof progress.statusCounts === 'object') {
    for (const [status, count] of Object.entries(progress.statusCounts)) {
      if (typeof count === 'number' && Number.isFinite(count) && count > 0) {
        statusCounts[status] = Math.round(count);
      }
    }
  }

  const noteCounts: Record<string, number> = {};
  if (progress?.noteCounts && typeof progress.noteCounts === 'object') {
    for (const [note, count] of Object.entries(progress.noteCounts)) {
      if (typeof count === 'number' && Number.isFinite(count) && count > 0) {
        noteCounts[note] = Math.round(count);
      }
    }
  }

  const totalSegments = Math.max(0, Math.round(safeNumber(progress?.totalSegments)));
  const translatedSegments = Math.max(0, Math.round(safeNumber(progress?.translatedSegments)));
  const audioSegments = Math.max(0, Math.round(safeNumber(progress?.audioSegments)));
  const completedSegments = Math.max(0, Math.round(safeNumber(progress?.completedSegments)));
  const translationPercent = Math.max(0, Math.min(100, Math.round(safeNumber(progress?.translationPercent))));
  const audioPercent = Math.max(0, Math.min(100, Math.round(safeNumber(progress?.audioPercent))));
  const completedPercent = Math.max(0, Math.min(100, Math.round(safeNumber(progress?.completedPercent))));
  const pendingReviewSegments = Math.max(0, Math.round(safeNumber(progress?.pendingReviewSegments)));
  const rating = Math.max(0, Math.min(5, Math.round(safeNumber(progress?.rating))));
  const notesTotalRaw = progress?.notesTotal;
  const notesTotal =
    typeof notesTotalRaw === 'number' && Number.isFinite(notesTotalRaw)
      ? Math.max(0, Math.round(notesTotalRaw))
      : Object.values(noteCounts).reduce((sum, value) => sum + value, 0);
  const updatedAt =
    progress && typeof progress.updatedAt === 'string' && progress.updatedAt.trim().length > 0
      ? progress.updatedAt
...
function progressSnapshotToStats(progress: ProjectProgressSnapshot): ProjectStats {
  return {
    totalSegments: progress.totalSegments,
    translatedSegments: progress.translatedSegments,
    audioSegments: progress.audioSegments,
    completedSegments: progress.completedSegments,
    translationPercent: progress.translationPercent,
    audioPercent: progress.audioPercent,
    completedPercent: progress.completedPercent,
    pendingReviewSegments: progress.pendingReviewSegments,
    statusCounts: { ...progress.statusCounts },
    noteCounts: { ...progress.noteCounts },
  };
}

function computeAggregatedStats(projects: DashboardProject[]): AggregatedStats {
  let aggregatedStats = createEmptyStats();
  let ratingSum = 0;

  for (const project of projects) {
    aggregatedStats = mergeProjectStats(aggregatedStats, project.stats);
    ratingSum += project.progress.rating;
  }

  const projectCount = projects.length;
  const notesTotal = Object.values(aggregatedStats.noteCounts).reduce((sum, value) => sum + value, 0);
  const averageRating = projectCount > 0 ? Math.round((ratingSum / projectCount) * 10) / 10 : 0;

  return {
    projectCount,
    totalSegments: aggregatedStats.totalSegments,
    translatedSegments: aggregatedStats.translatedSegments,
    audioSegments: aggregatedStats.audioSegments,
    completedSegments: aggregatedStats.completedSegments,
    pendingReviewSegments: aggregatedStats.pendingReviewSegments,
    notesTotal,
    averageRating,
    translationPercent: aggregatedStats.translationPercent,
    audioPercent: aggregatedStats.audioPercent,
    completedPercent: aggregatedStats.completedPercent,
    statusCounts: aggregatedStats.statusCounts,
    noteCounts: aggregatedStats.noteCounts,
  };
}

function getProgressBarClass(percent: number): string {
  if (percent >= 90) {
    return 'dashboard-progress__bar--success';
  }
  if (percent >= 60) {
    return 'dashboard-progress__bar--warning';
  }
  return 'dashboard-progress__bar--danger';
}

function createProgressElement(percent: number): HTMLElement {
  const wrapper = document.createElement('div');
  wrapper.className = 'dashboard-progress';
  const bar = document.createElement('div');
  bar.className = `dashboard-progress__bar ${getProgressBarClass(percent)}`;
...
function updateDashboardSummary(allProjects: DashboardProject[], visibleProjects: DashboardProject[]): void {
  if (!dashboardSummaryElement) {
    return;
  }

  const overallStats = computeAggregatedStats(allProjects);
  const visibleStats = computeAggregatedStats(visibleProjects);

  const summaryItems = dashboardSummaryElement.querySelectorAll<HTMLElement>('.project-dashboard__summary-item');
  summaryItems.forEach((item) => {
    switch (item.dataset.summary) {
      case 'projects': {
        if (allProjects.length === visibleProjects.length) {
          item.textContent = `Projekte: ${visibleProjects.length}`;
        } else {
          item.textContent = `Projekte: ${visibleProjects.length} / ${allProjects.length}`;
        }
        break;
      }
      case 'segments': {
        item.textContent = `Segmente: ${visibleStats.totalSegments}`;
        break;
      }
      case 'progress': {
        item.textContent = `Fortschritt: ${visibleStats.completedPercent} %`;
        break;
      }
      case 'review': {
        item.textContent = `Review offen: ${visibleStats.pendingReviewSegments}`;
        break;
      }
      default:
        break;
  ...
  if (dashboardElement) {
    dashboardElement.dataset.totalProjects = String(overallStats.projectCount);
  }
}

function matchesDashboardFilter(project: DashboardProject): boolean {
  const stats = project.stats;
  switch (dashboardFilter) {
    case 'open':
      return stats.completedPercent < 100;
    case 'review':
      return stats.pendingReviewSegments > 0;
    case 'done':
      return stats.completedPercent >= 100;
    default:
      return true;
...
function getDashboardProjects(): DashboardProject[] {
  return projectLibraryEntries.map((entry) => {
    const progress = normalizeProgressSnapshot(entry.manifest.progress);
    return {
      entry,
      chapter: sanitizeChapterLabel(entry.manifest.chapter),
      level: sanitizeLevelLabel(entry.manifest.level),
      progress,
      stats: progressSnapshotToStats(progress),
    };
  });
}

function setDashboardEmptyState(visible: boolean, message?: string): void {
  if (!dashboardEmptyElement) {
    return;
  }
  if (visible) {
    if (message) {
      dashboardEmptyElement.textContent = message;
    }
    dashboardEmptyElement.style.display = 'block';
  } else {
    dashboardEmptyElement.style.display = 'none';
  }
}
...
function renderProjectDashboard(): void {
  ...
  const visibleProjects = projects.filter((project) => matchesDashboardFilter(project));

  updateDashboardSummary(projects, visibleProjects);
  updateFilterButtons();

  if (projects.length === 0) {
    dashboardListElement.innerHTML = '';
    setDashboardEmptyState(true, 'Noch keine Projekte vorhanden.');
    return;
  }

  if (visibleProjects.length === 0) {
    dashboardListElement.innerHTML = '';
    setDashboardEmptyState(true, 'Keine Projekte für den aktuellen Filter.');
    return;
  }
  ...
  const chapterMap = new Map<string, Map<string, DashboardProject[]>>();
  ...
  for (const project of visibleProjects) {
    ...
    const levelMap = chapterMap.get(chapter)!;
    const list = levelMap.get(level) ?? [];
    list.push(project);
    levelMap.set(level, list);
  }

  const sortedChapters = Array.from(chapterMap.keys()).sort((a, b) => a.localeCompare(b, 'de', { sensitivity: 'base' }));

  for (const chapterName of sortedChapters) {
    const levelMap = chapterMap.get(chapterName)!;
    const chapterProjects: DashboardProject[] = [];
    levelMap.forEach((list) => chapterProjects.push(...list));
    const chapterStats = computeAggregatedStats(chapterProjects);

    const chapterElement = document.createElement('section');
    chapterElement.className = 'dashboard-chapter';
    chapterElement.dataset.chapter = chapterName;

    const chapterHeader = document.createElement('div');
    chapterHeader.className = 'dashboard-chapter__header';

    const chapterTitle = document.createElement('h4');
    chapterTitle.className = 'dashboard-chapter__title';
    chapterTitle.textContent = `Kapitel: ${chapterName}`;
    chapterHeader.appendChild(chapterTitle);

    const chapterMeta = document.createElement('div');
    chapterMeta.className = 'dashboard-meta-line';

    const projectCountSpan = document.createElement('span');
    projectCountSpan.textContent = `Projekte: ${chapterStats.projectCount}`;
    chapterMeta.appendChild(projectCountSpan);

    const chapterProgressSpan = document.createElement('span');
    chapterProgressSpan.textContent = `Fertig: ${chapterStats.completedPercent}%`;
    chapterMeta.appendChild(chapterProgressSpan);

    const ratingWrapper = document.createElement('span');
    const ratingLabel = document.createElement('span');
    ratingLabel.textContent = 'Ø Bewertung: ';
    const ratingStars = document.createElement('span');
    ratingStars.className = 'dashboard-rating';
    ratingStars.textContent = formatRatingStars(chapterStats.averageRating);
    const ratingValue = document.createElement('span');
    ratingValue.textContent = ` (${chapterStats.averageRating.toFixed(1)})`;
    ratingWrapper.append(ratingLabel, ratingStars, ratingValue);
    chapterMeta.appendChild(ratingWrapper);

    if (chapterStats.pendingReviewSegments > 0) {
      const reviewBadge = document.createElement('span');
      reviewBadge.className = 'dashboard-review-indicator';
      reviewBadge.textContent = `Review ${chapterStats.pendingReviewSegments}`;
      chapterMeta.appendChild(reviewBadge);
    }

    chapterHeader.appendChild(chapterMeta);
    chapterElement.appendChild(chapterHeader);
    chapterElement.appendChild(createProgressElement(chapterStats.completedPercent));

    const levelsContainer = document.createElement('div');
    levelsContainer.className = 'dashboard-levels';

    const sortedLevels = Array.from(levelMap.keys()).sort((a, b) => a.localeCompare(b, 'de', { sensitivity: 'base' }));

    for (const levelName of sortedLevels) {
      const levelProjects = levelMap.get(levelName)!;
      levelProjects.sort((a, b) => {
        const nameA = a.entry.manifest.name ?? a.entry.folderName;
        const nameB = b.entry.manifest.name ?? b.entry.folderName;
        return nameA.localeCompare(nameB, 'de', { sensitivity: 'base' });
      });

      const levelStats = computeAggregatedStats(levelProjects);

      const levelElement = document.createElement('section');
      levelElement.className = 'dashboard-level';
      levelElement.dataset.chapter = chapterName;
      levelElement.dataset.level = levelName;
      levelElement.addEventListener('dragover', (event) => handleLevelDragOver(event));
      levelElement.addEventListener('dragenter', () => levelElement.classList.add('is-drop-target'));
      levelElement.addEventListener('dragleave', () => levelElement.classList.remove('is-drop-target'));
      levelElement.addEventListener('drop', (event) => handleLevelDrop(event, chapterName, levelName, levelElement));

      const levelHeader = document.createElement('div');
      levelHeader.className = 'dashboard-level__header';

      const titleWrapper = document.createElement('div');
      titleWrapper.className = 'dashboard-level__title';
      titleWrapper.textContent = `Level: ${levelName}`;
      levelHeader.appendChild(titleWrapper);

      const badges = document.createElement('div');
      badges.className = 'dashboard-level__badges';

      const projectCountBadge = document.createElement('span');
      projectCountBadge.textContent = `Projekte: ${levelProjects.length}`;
      badges.appendChild(projectCountBadge);

      const translationBadge = document.createElement('span');
      translationBadge.textContent = `Übersetzung: ${levelStats.translationPercent}%`;
      badges.appendChild(translationBadge);

      const audioBadge = document.createElement('span');
      audioBadge.textContent = `Audio: ${levelStats.audioPercent}%`;
      badges.appendChild(audioBadge);

      const reviewBadge = document.createElement('span');
      reviewBadge.textContent = `Review: ${levelStats.pendingReviewSegments}`;
      badges.appendChild(reviewBadge);

      levelHeader.appendChild(badges);
      levelHeader.addEventListener('contextmenu', (event) => openLevelContextMenu(event, levelName, levelStats));

      levelElement.appendChild(levelHeader);
      levelElement.appendChild(createProgressElement(levelStats.completedPercent));

      const projectsContainer = document.createElement('div');
      projectsContainer.className = 'dashboard-projects';

      for (const project of levelProjects) {
        const progress = project.progress;
        const card = document.createElement('article');
        card.className = 'dashboard-project';
        card.dataset.path = project.entry.path;
        card.draggable = true;

        if (project.entry.path === selectedProjectPath) {
          card.classList.add('is-selected');
        }

        if (project.entry.hasLock) {
          card.classList.add('is-locked');
        }

        const cardHeader = document.createElement('div');
        cardHeader.className = 'dashboard-project__header';

        const nameSpan = document.createElement('span');
        nameSpan.className = 'dashboard-project__name';
        nameSpan.textContent = project.entry.manifest.name ?? project.entry.folderName;
        cardHeader.appendChild(nameSpan);

        const summarySpan = document.createElement('span');
        summarySpan.className = 'dashboard-project__badge';
        summarySpan.textContent = `Σ ${progress.completedPercent}%`;
        cardHeader.appendChild(summarySpan);

        card.appendChild(cardHeader);

        const statsLine = document.createElement('div');
        statsLine.className = 'dashboard-project__stats';

        const translationStat = document.createElement('span');
        translationStat.textContent = `Übersetzung ${progress.translationPercent}%`;
        statsLine.appendChild(translationStat);

        const audioStat = document.createElement('span');
        audioStat.textContent = `Audio ${progress.audioPercent}%`;
        statsLine.appendChild(audioStat);

        if (progress.pendingReviewSegments > 0) {
          const reviewStat = document.createElement('span');
          reviewStat.className = 'dashboard-review-indicator';
          reviewStat.textContent = `Review ${progress.pendingReviewSegments}`;
          statsLine.appendChild(reviewStat);
        }

        card.appendChild(statsLine);

        const metaLine = document.createElement('div');
        metaLine.className = 'dashboard-project__meta';
        ...
        const ratingStars = document.createElement('span');
        ratingStars.className = 'dashboard-rating';
        ratingStars.textContent = formatRatingStars(progress.rating);
        const ratingValue = document.createElement('span');
        ratingValue.textContent = ` (${progress.rating.toFixed(0)})`;
        ratingWrapper.append(ratingLabel, ratingStars, ratingValue);
        metaLine.appendChild(ratingWrapper);

        const segmentInfo = document.createElement('span');
        segmentInfo.textContent = `Segmente: ${progress.totalSegments}`;
        metaLine.appendChild(segmentInfo);

        const updatedInfo = document.createElement('span');
        updatedInfo.textContent = `Aktualisiert: ${formatTimestamp(project.entry.manifest.updatedAt)}`;
        metaLine.appendChild(updatedInfo);

        card.appendChild(metaLine);

        card.title =
          `${project.entry.manifest.name ?? project.entry.folderName}\n` +
          `Kapitel: ${project.chapter} · Level: ${project.level}\n` +
          `Fortschritt: ${progress.completedPercent}% (Übersetzung ${progress.translationPercent}% · Audio ${progress.audioPercent}%)\n` +
          `Segmente: ${progress.totalSegments} · Review offen: ${progress.pendingReviewSegments}`;

        card.addEventListener('click', () => handleProjectSelection(project));
        card.addEventListener('contextmenu', (event) => openProjectContextMenu(event, project));
        card.addEventListener('dragstart', (event) => handleProjectDragStart(event, project));
        card.addEventListener('dragend', handleProjectDragEnd);

        projectsContainer.appendChild(card);
      }

      levelElement.appendChild(projectsContainer);
      levelsContainer.appendChild(levelElement);
    }

    chapterElement.appendChild(levelsContainer);
    dashboardListElement.appendChild(chapterElement);
  }
}
...
function handleProjectSelection(project: DashboardProject): void {
  closeDashboardContextMenu();
  setSelectedProject(project.entry.path, project.entry);
}

function handleProjectDragStart(event: DragEvent, project: DashboardProject): void {
  draggedProjectPath = project.entry.path;
  if (event.dataTransfer) {
    event.dataTransfer.setData('text/plain', project.entry.path);
...
function handleProjectDragEnd(): void {
  ...
  const targets = document.querySelectorAll<HTMLElement>('.dashboard-level.is-drop-target');
...
function handleLevelDrop(
  ...
  const transferredPath = event.dataTransfer?.getData('text/plain');
  const path = transferredPath || draggedProjectPath;
  if (!path) {
    return;
  }

  void moveProjectToLevel(path, chapter, level);
}

async function moveProjectToLevel(path: string, chapter: string, level: string): Promise<void> {
  if (!window.projectLibrary) {
    setProjectInfo('Projektbibliothek steht nicht zur Verfügung.');
    return;
  }

  const sanitizedChapter = sanitizeChapterLabel(chapter);
  const sanitizedLevel = sanitizeLevelLabel(level);

  try {
    await window.projectLibrary.update(path, { chapter: sanitizedChapter, level: sanitizedLevel });
    await loadProjectLibrary();
    setProjectInfo(`Projekt verschoben nach ${sanitizedChapter} · ${sanitizedLevel}.`);
    setSelectedProject(path);
  } catch (error) {
    console.error('Projekt konnte nicht verschoben werden:', error);
    setProjectInfo(`Projekt konnte nicht verschoben werden: ${(error as Error).message}`);
  }
}

async function updateProjectMetadataEntry(
  path: string,
  ...
  try {
    await window.projectLibrary.update(path, updates);
    await loadProjectLibrary();
    setProjectInfo(successMessage);
    setSelectedProject(path);
  } catch (error) {
    console.error('Metadaten konnten nicht aktualisiert werden:', error);
    setProjectInfo(`Metadaten konnten nicht aktualisiert werden: ${(error as Error).message}`);
  }
}

function openProjectContextMenu(event: MouseEvent, project: DashboardProject): void {
  event.preventDefault();
  closeDashboardContextMenu();
  ...
  const addItem = (label: string, handler: () => void) => {
    ...
    item.className = 'dashboard-context-menu__item';
  ...
  addItem('Projekt auswählen', () => handleProjectSelection(project));
  addItem('Projekt öffnen', () => {
  ...
  addItem('Kapitel zuweisen …', () => {
    const current = project.chapter;
    const input = window.prompt('Neues Kapitel festlegen', current);
    if (input === null) {
      return;
    }
    const sanitized = sanitizeChapterLabel(input);
    if (sanitized === current) {
      setProjectInfo('Keine Änderung vorgenommen.');
      return;
    }
    void updateProjectMetadataEntry(
      project.entry.path,
      { chapter: sanitized },
      `Kapitel geändert zu ${sanitized}.`
    );
  });

  addItem('Level zuweisen …', () => {
    const current = project.level;
    const input = window.prompt('Neues Level festlegen', current);
    if (input === null) {
      return;
    }
    const sanitized = sanitizeLevelLabel(input);
    if (sanitized === current) {
      setProjectInfo('Keine Änderung vorgenommen.');
      return;
    }
    void updateProjectMetadataEntry(
      project.entry.path,
      { level: sanitized },
      `Level geändert zu ${sanitized}.`
    );
  });

  addItem('Bewertung setzen …', () => {
    const progress = project.progress;
    const input = window.prompt('Neue Bewertung (0–5)', String(progress.rating));
    if (input === null) {
      return;
    }
    const value = Number.parseInt(input, 10);
    if (Number.isNaN(value)) {
      setProjectInfo('Die eingegebene Bewertung ist ungültig.');
      return;
    }
    const clamped = Math.max(0, Math.min(5, value));
    if (clamped === progress.rating) {
      setProjectInfo('Bewertung bleibt unverändert.');
      return;
    }
    void updateProjectMetadataEntry(
      project.entry.path,
      { rating: clamped },
      `Bewertung aktualisiert (neuer Wert: ${clamped}).`
    );
  });

  addItem('Review-Statistiken anzeigen', () => showProjectReviewDialog(project));
  addItem('Notizübersicht anzeigen', () => showProjectNotesDialog(project));

  document.body.appendChild(menu);
  dashboardContextMenu = menu;

  positionContextMenu(menu, event.clientX, event.clientY);
}

function openLevelContextMenu(event: MouseEvent, levelName: string, stats: AggregatedStats): void {
  event.preventDefault();
  closeDashboardContextMenu();
  ...
  addItem('Level-Statistiken anzeigen', () => showLevelDialog(levelName, stats));
  addItem('Notizübersicht anzeigen', () => showLevelNotesDialog(levelName, stats));
  addItem('Review-Filter aktivieren', () => {
    setDashboardFilter('review');
    setProjectInfo(`Review-Filter für Level ${levelName} aktiviert.`);
  });
...
function positionContextMenu(menu: HTMLElement, clientX: number, clientY: number): void {
  ...
  const left = Math.min(clientX, Math.max(viewportPadding, maxLeft));
  const top = Math.min(clientY, Math.max(viewportPadding, maxTop));
...
function closeDashboardContextMenu(): void {
  if (!dashboardContextMenu) {
    return;
  }
  dashboardContextMenu.remove();
...
function handleContextMenuOutside(event: Event): void {
  if (!dashboardContextMenu) {
    return;
  }
  if (event.target instanceof Node && dashboardContextMenu.contains(event.target)) {
    return;
  }
  closeDashboardContextMenu();
}
...
function openDialog(title: string, content: HTMLElement): void {
  closeDashboardContextMenu();
  ...
  buttonBar.className = 'dialog-buttons';
...
function showProjectReviewDialog(project: DashboardProject): void {
  const progress = project.progress;
  const container = document.createElement('div');

  const summary = document.createElement('div');
  summary.textContent = `Offene Review-Segmente: ${progress.pendingReviewSegments}`;
  container.appendChild(summary);

  if (Object.keys(progress.statusCounts).length > 0) {
    const statusList = document.createElement('div');
    Object.entries(progress.statusCounts)
      .sort((a, b) => b[1] - a[1])
      .forEach(([status, count]) => {
        const row = document.createElement('div');
        row.textContent = `${status}: ${count}`;
        statusList.appendChild(row);
      });
    container.appendChild(statusList);
  } else {
    const noStatus = document.createElement('div');
    noStatus.textContent = 'Keine Statuswerte verfügbar.';
    container.appendChild(noStatus);
  }

  openDialog(`Review-Statistiken · ${project.entry.manifest.name ?? project.entry.folderName}`, container);
}

function showProjectNotesDialog(project: DashboardProject): void {
  const progress = project.progress;
  const container = document.createElement('div');

  const summary = document.createElement('div');
  summary.textContent = `Notizen insgesamt: ${progress.notesTotal}`;
  container.appendChild(summary);

  if (Object.keys(progress.noteCounts).length > 0) {
    const noteList = document.createElement('div');
    Object.entries(progress.noteCounts)
      .sort((a, b) => b[1] - a[1])
      .forEach(([note, count]) => {
        const row = document.createElement('div');
        row.textContent = `${note}: ${count}`;
        noteList.appendChild(row);
      });
    container.appendChild(noteList);
  } else {
    const noNotes = document.createElement('div');
    noNotes.textContent = 'Keine Notizen vorhanden.';
    container.appendChild(noNotes);
  }

  openDialog(`Notizübersicht · ${project.entry.manifest.name ?? project.entry.folderName}`, container);
}

function showLevelDialog(levelName: string, stats: AggregatedStats): void {
  const container = document.createElement('div');

  const summary = document.createElement('div');
  summary.textContent = `Projekte: ${stats.projectCount} · Segmente: ${stats.totalSegments} · Review: ${stats.pendingReviewSegments}`;
  container.appendChild(summary);
  ...
  openDialog(`Level-Statistiken · ${levelName}`, container);
}

function showLevelNotesDialog(levelName: string, stats: AggregatedStats): void {
  const container = document.createElement('div');

  const summary = document.createElement('div');
  summary.textContent = `Notizen insgesamt: ${stats.notesTotal}`;
  container.appendChild(summary);

  if (Object.keys(stats.noteCounts).length > 0) {
    const noteList = document.createElement('div');
  ...
  openDialog(`Notizübersicht · ${levelName}`, container);
}

function handleRandomProject(reviewOnly: boolean): void {
  const projects = getDashboardProjects();
  const candidates = reviewOnly
    ? projects.filter((project) => project.stats.pendingReviewSegments > 0)
    : projects.filter((project) => matchesDashboardFilter(project));

  if (candidates.length === 0) {
    setProjectInfo(reviewOnly ? 'Keine Projekte mit offenen Reviews gefunden.' : 'Keine Projekte verfügbar.');
    return;
  }

  const randomIndex = Math.floor(Math.random() * candidates.length);
  const project = candidates[randomIndex];
  handleProjectSelection(project);
  focusProjectCard(project.entry.path);
}

function focusProjectCard(projectPath: string): void {
  if (!dashboardListElement) {
    return;
  }
  const selector = `.dashboard-project[data-path="${escapeSelector(projectPath)}"]`;
  const card = dashboardListElement.querySelector<HTMLElement>(selector);
  if (card) {
...
function updateFilterButtons(): void {
  if (!dashboardFiltersElement) {
    return;
  }
  const buttons = dashboardFiltersElement.querySelectorAll<HTMLButtonElement>('button[data-filter]');
  buttons.forEach((button) => {
    if (button.dataset.filter === dashboardFilter) {
...
function setDashboardFilter(filter: DashboardFilter): void {
...
function handleFilterClick(event: Event): void {
  ...
  const filter = button.dataset.filter as DashboardFilter | undefined;
  if (!filter) {
    return;
  }
...
async function loadProjectLibrary(): Promise<void> {
  ...
  try {
    ...
    renderProjectLibrary();
    setDashboardEmptyState(true, `Fehler beim Laden der Projektliste: ${(error as Error).message}`);
  }
}
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (dashboardFiltersElement) {
    dashboardFiltersElement.addEventListener('click', (event) => handleFilterClick(event));
  }

  if (dashboardRandomButton) {
    dashboardRandomButton.addEventListener('click', () => handleRandomProject(false));
  }

  if (dashboardRandomReviewButton) {
    dashboardRandomReviewButton.addEventListener('click', () => handleRandomProject(true));
  }
