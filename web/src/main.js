...
async function timeStretchBuffer(buffer, factor) {
    ...
    // Nur die gestretchte Stille entfernen, damit nichts verloren geht
    const cut = Math.round(pad * factor);
    const len = Math.max(out.length - cut * 2, 0);
    const trimmed = ctx.createBuffer(out.numberOfChannels, len, out.sampleRate);
    for (let ch = 0; ch < out.numberOfChannels; ch++) {
        trimmed.getChannelData(ch).set(out.getChannelData(ch).subarray(cut, cut + len));
    }
    ctx.close();
    return trimmed;
}
