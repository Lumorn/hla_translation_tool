...
async function timeStretchBuffer(buffer, factor) {
    ...
    const sr = buffer.sampleRate;
    // etwas Stille an Anfang und Ende anhängen, um Abschneiden zu vermeiden
    const pad = Math.round(sr * 0.05);
    const ctxPad = new (window.AudioContext || window.webkitAudioContext)();
    const padded = ctxPad.createBuffer(buffer.numberOfChannels, buffer.length + pad * 2, sr);
    for (let ch = 0; ch < buffer.numberOfChannels; ch++) {
        padded.getChannelData(ch).set(buffer.getChannelData(ch), pad);
    }
    ctxPad.close();
    ...
    // Gestreckte Stille anhand des Faktors entfernen
    const padAfter = Math.round(pad / factor); // Laenge der gepolsterten Stille nach dem Stretch
    const start = padAfter;
    let len = Math.max(out.length - padAfter * 2, 0);
    const target = Math.round(buffer.length / factor);
    len = Math.min(len, target);
    const trimmed = ctx.createBuffer(out.numberOfChannels, len, out.sampleRate);
    for (let ch = 0; ch < out.numberOfChannels; ch++) {
        trimmed.getChannelData(ch).set(out.getChannelData(ch).subarray(start, start + len));
    }
    ctx.close();
    return trimmed;
}
