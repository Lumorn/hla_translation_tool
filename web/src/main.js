...
async function timeStretchBuffer(buffer, factor) {
    ...
    const sr = buffer.sampleRate;
    // mehr Stille anhängen, damit der Algorithmus sauber arbeiten kann
    const pad = Math.round(sr * 0.1); // 100 ms pro Seite
    ...
    const padAfter = Math.round(pad / factor); // Länge der gepolsterten Stille nach dem Stretch
