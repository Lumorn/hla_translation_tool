<template>
    <!-- Platzhalter: TD wird per JS erzeugt -->
</template>
<script>
...
export function scoreCellTemplate(file, escapeHtml) {
    const cls = file.score >= 70 ? 'score-high' : file.score >= 40 ? 'score-medium' : 'score-low';
    ...
    return `<td class="score-cell ${cls}" data-suggestion="${sug}" data-comment="${com}" title="${title}">${file.score ?? ''}</td>`;
}
